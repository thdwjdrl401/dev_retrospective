...
## 1. 포트 비개방 원격 DB 백업 — SSH(JSch) + 계정 권한 분리
...
보안에서 신경 쓴 건 **권한 분리**입니다. SSH 접속 계정(OS)과 `mysqldump` 실행 계정(DB)을 나누고, 서버별(R1·R3·R5·TOMAS)로 자격증명을 따로 뒀습니다 — 한 계정이 뚫려도 다른 서버·다른 권한으로 번지지(lateral movement) 않게 했습니다. 백업 파일은 Google Drive API(OAuth2, `@PostConstruct`로 앱 시작 시 1회 인증)로 올리고 **업로드 성공 시에만 로컬에서 삭제**해 이중 안전성을 뒀습니다. MIME 타입은 확장자가 아니라 Apache Tika의 매직넘버 분석으로 감지했습니다.
...
**운영 디테일을 챙겼습니다.** 권한·자격증명 분리, 업로드 성공 시에만 삭제, 상태 전환 시점 알림, 차등 백업 — 무인 자동화가 조용히 실패하지 않도록 설계했습니다.
...
**자격증명을 평문으로 뒀습니다.** SSH·DB 비밀번호를 `Const`·`application.properties`에 평문으로 저장했습니다. 환경변수나 Vault로 외부화하고, SSH는 비밀번호 대신 key 인증으로 갔어야 했습니다. `echo {pw} | sudo -S` 패턴도 `ps aux`로 노출될 수 있어 `~/.my.cnf` + `--defaults-extra-file`이 더 안전합니다.

**`StrictHostKeyChecking=no`는 MITM 위험이 있습니다.** 자동화 편의로 껐지만, `known_hosts` 사전 등록이 정답입니다.
